...
import {
  ...
  deleteInstallRequest, dismissJobScheduling, getDismissedSchedulingIds,
} from '../store/data';
...
export default function InstallationCalendar() {
  ...
  const handleDeleteInstall = () => {
    ...
    deleteInstallRequest(selectedInstall.id); // remove only this booking, NOT the whole job
          ...
          <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-4">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 space-y-4">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <h3 className="font-bold text-slate-900">Remove this booking?</h3>
                  <p className="text-sm text-slate-500 mt-1">
                    This removes the installation booking for <strong>{cust?.name || 'this job'}</strong>{job?.jobNumber ? ` (${job.jobNumber})` : ''} from the calendar. The job and everything else on it stay.
                  </p>
                </div>
              </div>
              <div className="flex gap-3">
                ...
                <button onClick={handleDeleteInstall} className="flex-1 bg-red-500 hover:bg-red-600 text-white text-sm font-semibold py-2.5 rounded-xl">Remove booking</button>
              </div>
            </div>
          </div>
        );
